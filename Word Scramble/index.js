...
function getNext(varName) {
    ...
    if (localStorage.length == 0) {
        ...
        if (varName == "name") {
            x = varName + ((localStorage.length + 3) / 3);
            return x;
        }
        if (varName == "score") {
            x = varName + ((localStorage.length + 2) / 3);
            return x;
        }
        if (varName == "date") {
            x = varName + ((localStorage.length + 1) / 3);
            return x;
        }
    }
}
...
function storeData(name, points) {
    ...
    if (storageLength < MAXIMUM_ENTRIES && !names.includes(name)) {
        localStorage.setItem(getNext("name"), name);
        localStorage.setItem(getNext("score"), points);
        localStorage.setItem(getNext("date"), new Date().getTime() / 1000);
    } else {
        if (names.includes(name)) {
            if (points > scores[names.indexOf(name)]) {
                localStorage.setItem("score" + (names.indexOf(name) + 1), points);
            }
        } else {
            ...
            for (var i = 0; i < scores.length; i++) {
                if (points > scores[i]) {
                    ...
                    localStorage.setItem("name" + (dates.indexOf(recentDate) + 1), name);
                    localStorage.setItem("score" + (dates.indexOf(recentDate) + 1), points);
                    localStorage.setItem("date" + (dates.indexOf(recentDate) + 1), new Date().getTime() / 1000);
                    break;
                }
            }
        }
